---
title: "Disponibilidade de leitos hospitalares nos municípios do RS: desafios no enfrentamento do COVID-19"
output: 
...
leitos.cnes <- fetch_datasus(year_start = 2020, month_start = 6,
                              year_end = 2020, month_end = 6,
                              uf = "RS",
...
x.etaria.tot$P.Faixa.de00a14anos = (x.etaria.tot$P.Faixa.de00a04anos+
                                            x.etaria.tot$P.Faixa.de05a09ano+
                                            x.etaria.tot$P.Faixa.de10a14anos)
x.etaria.tot$T.Faixa.de00a14anos = round(x.etaria.tot$P.Faixa.de00a14anos * x.etaria.tot$POPULACAO,0)
...
x.etaria.tot$P.adultos = 1 - x.etaria.tot$P.Faixa.de00a14anos
x.etaria.tot$T.adultos  = round(x.etaria.tot$P.adultos * x.etaria.tot$POPULACAO,0)
...
equip.cnes <- fetch_datasus(year_start = 2020, month_start = 6,
                              year_end = 2020, month_end = 6,
                              uf = "RS",
                              information_system = "CNES-EQ")
...
# EQUIPQMENTOS RESPIRADORES MUNICIPIOS
tab.equip.resp.mun = equip.cnes %>% 
    dplyr::select("CODUFMUN","MUNICIPIO","QTEXIST","QTUSO","INDSUS","INDNSUS","POPULACAO","REGSAUDE","EQUIPAMENTO") %>%
    filter(str_starts(EQUIPAMENTO, "RESPIRADOR/VENTILADOR")) %>%
...
tab.equip.mun  = x.pop2020corede %>%
  left_join(tab.equip.resp.mun, by = "CODUFMUN") %>%
...
# internacoes.sih = fetch_datasus(year_start = 2020, month_start = 2,
#               year_end = 2020, month_end = 2,
#               uf = c("RS"),
...
<!-- ```{r, echo=FALSE, out.width='100%', out.height='900px', results='asis'} -->
<!-- library(knitr) -->
<!-- knitr::include_graphics("ArtigoLeitosRS_NOVO.pdf") -->
<!-- ``` -->

Competência: Junho/2020
...
Mapa Municípios Hospitais {data-navmenu="Mapas Municípios"}
=======================================================================

Column
-----------------------------------------------------------------------
### Mapa hospitais por município por porte

```{r mapaproblematico, echo=FALSE, message=FALSE, warning=FALSE, include=TRUE}
...
Column
-----------------------------------------------------------------------
### Mapa QL leitos existentes, SUS e Não-SUS nos municípios

```{r echo=FALSE}
mapars.ql = tm_shape(mapars.leitos)+
tm_fill(c("QL.QTEXIST.na"
          # ,"QL.QTSUS.na"
          # ,"QL.QTNSUS.na"
          ), auto.palette.mapping=FALSE, id="MUNICIPIO",
        ...
        breaks=c(-Inf,1,2,Inf), textNA = "Zero", colorNA = "red",
          popup.vars = c("QL.QTEXIST"= "QL.QTEXIST.na",
                         "QL.QTSUS"= "QL.QTSUS.na",
                         "QL.QTNSUS" = "QL.QTNSUS.na"),
        palette="Blues")+
...
tmap_leaflet(mapars.ql) %>% clearControls()
...
Column
-----------------------------------------------------------------------
### Mapa leitos por 10.00 mil hab. nos municípios

```{r, echo=FALSE, warning=FALSE}
mapars.leitos.mil1 = tm_shape(mapars.leitos)+
tm_fill(c("TOTAL10MIL.na"
          # , "TOTAL10MIL.SUS.na"
          # ,"TOTAL10MIL.NSUS.na"
          ), auto.palette.mapping=FALSE, id="MUNICIPIO",
          ...
          palette="Blues",
        popup.vars = c("TOTAL10MIL"= "TOTAL10MIL.na",
                       "TOTAL10MIL.SUS"= "TOTAL10MIL.SUS.na",
                       "TOTAL10MIL.NSUS" = "TOTAL10MIL.NSUS.na"),
          style="kmeans")+
tm_layout(frame = FALSE)+
tm_compass(position=c("right", "bottom"))+
tm_scale_bar(position=c("right", "bottom"))+
...
tmap_leaflet(mapars.leitos.mil1) %>% clearControls()
...
tmap_leaflet(mapa.leitos.tipo.uti.ad1) %>% clearControls()
...
eta.criancas = ggplot(mapa.leitos.tipo.uti.ad, aes(y=P.Faixa.de00a14anos, x=log(POPULACAO)))+
  ggtitle("")+
  geom_point()+
  labs(y = "Proporção crianças (0 a 14 anos)", x = "log(População)")+
  # geom_text(aes(label=MUNICIPIO),hjust=0, vjust=0)
  ...
  #              parse = TRUE,
  #              label.y = .95, label.x = .95)
  # stat_cor(label.y = .95, label.x = .95) +
stat_cor(method = "pearson")


eta.adultos = ggplot(mapa.leitos.tipo.uti.ad, aes(y=P.adultos, x=log(POPULACAO)))+
  ggtitle("")+
  geom_point()+
  labs(y = "Proporção adultos (15 anos ou mais)", x = "log(População)")+
  # geom_text(aes(label=MUNICIPIO),hjust=0, vjust=0)
  geom_smooth(method = "lm")+
  # stat_poly_eq(formula = my.formula,
  #              eq.with.lhs = "italic(hat(y))~`=`~",
  ...
  #              parse = TRUE,
  #              label.y = .95, label.x = .95)
  # stat_cor(label.y = .95, label.x = .95) #this means at 35th unit in the y axis, the r squared and p value will be shown
  #       stat_regline_equation(label.y = 0.7) 
stat_cor(method = "pearson")
  
eta.idosos  = ggplot(mapa.leitos.tipo.uti.ad, aes(y=P.Faixa.de60anosemais, x=log(POPULACAO)))+
  ggtitle("")+
  geom_point()+
  labs(y = "Proporção idosos (60 anos ou mais)", x = "log(População)")+
  # geom_text(aes(label=MUNICIPIO),hjust=0, vjust=0)
...
library(ggpubr)
ggarrange(eta.criancas,  eta.adultos,  eta.idosos +
            rremove("x.text"), 
           labels = c("A", "B", "C"),
          ncol = 3, nrow = 1)
...
```{r, echo=FALSE}
mapa.leitos.tipo.uti.ad.eta1 = 
  tm_shape(mapa.leitos.tipo.uti.ad)+
  tm_fill(c("P.Faixa.de00a14anos"
             ,"P.adultos"
             ,"P.Faixa.de60anosemais"
            ), auto.palette.mapping=FALSE, id="MUNICIPIO",
          #style="kmeans",
          # popup.vars = c("QTEXIST"= "QTEXIST",
          #                "QTSUS"= "QTSUS",
          #                "QTNSUS" = "QTNSUS"),
          # breaks=c(quantile(mapa.leitos.tipo.uti.ad$P.Faixa.de00a14anos, na.rm=TRUE),
          #          quantile(mapa.leitos.tipo.uti.ad$P.adultos, na.rm=TRUE),
          #          quantile(mapa.leitos.tipo.uti.ad$P.Faixa.de60anosemais, na.rm=TRUE)), textNA = "Zero", colorNA = "red",
          # breaks = c(-Inf, 0.05, 0.10, 0.20, 0.70, 0.8, Inf),
          title=c("Proporção crianças (0 a 14 anos)",
                  "Proporção adultos (15 anos ou mais)", 
                  "Proporção idosos (60 anos ou mais)"
                  ),
          # palette="Blues",
          palette=c("Oranges"),
          style="cont",
  ...
  tm_legend(legend.format = list(text.separator= "a", 
                                 text.or.more="ou maior",
                                 text.less.than="menor que"
                                 # ,fun=function(x) formatC(x, digits=2, format="d")
                                 ),
            position=c("left", "top"), compass.type="8star")+
    tm_shape(mapas.leitos.regsaude) +
  tm_borders("black", lwd=2)
...
# jpeg("mapa.leitos.tipo.uti.ad.eta1.jpeg", units="in", width=15, height=5, res=600)
# mapa.leitos.tipo.uti.ad.eta1
# dev.off()
...
```{r, include=FALSE}
tab.etaria.regsaude = x.etaria.tot %>%
  dplyr::select("MUNICIPIO", "POPULACAO", "REGSAUDE","T.adultos", "T.Faixa.de00a14anos", "T.Faixa.de60anosemais") %>%
  group_by(REGSAUDE) %>%
  summarise("T.adultos"  = sum(T.adultos),
            "T.Faixa.de00a14anos"  = sum(T.Faixa.de00a14anos),
            "T.Faixa.de60anosemais"  = sum(T.Faixa.de60anosemais),
            "POPULACAO" = sum(unique(as.numeric(POPULACAO)))
            )%>%
  mutate("P.adultos" = round(T.adultos*100 / POPULACAO,2),
         "P.Faixa.de00a14anos" = round(T.Faixa.de00a14anos*100 / POPULACAO,2),
         "P.Faixa.de60anosemais" = round(T.Faixa.de60anosemais*100 / POPULACAO,2))
    
 # View(tab.etaria.regsaude)  
```

Faixas etárias {data-navmenu="Mapas Regiões de Saúde"}
=======================================================================

Column
-----------------------------------------------------------------------
### Mapa Faixas etárias Regiões de Saúde


```{r, echo=FALSE}
library(spdplyr)
mapa.etaria.regsaude = merge(mapas.leitos.regsaude, tab.etaria.regsaude, by="REGSAUDE", all.x=T)

mapa.etaria.regsaude1  = tm_shape(mapars.leitos, palette="Greys") +
  tm_borders("gray", lwd=0.1)+
  tm_shape(mapa.etaria.regsaude)+
  tm_fill(c("P.Faixa.de00a14anos","P.adultos","P.Faixa.de60anosemais"), auto.palette.mapping=FALSE, id="REGSAUDE",
          title=c("% crianças (0 a 14 anos)"
                  ,"% adultos (15 anos ou mais)"
                  ,"% idosos (60 anos ou mais)"),
           # breaks=quantile(mapa.etaria.regsaude$P.adultos, na.rm=TRUE), textNA = "Zero", colorNA = "red",
          palette="Oranges", alpha = .6,
          style="cont")+
  tm_layout(frame = FALSE)+
  tm_compass(position=c("right", "bottom"))+
  tm_scale_bar(position=c("left", "bottom"))+
  tm_text(c("P.Faixa.de00a14anos","P.adultos","P.Faixa.de60anosemais"), scale=.6 )+
  tm_borders(alpha=.5)+
  tm_layout(
    main.title.position = 'left',
    main.title.size=1)+
  tm_legend(legend.format = list(text.separator= "a",
                                 text.or.more="ou maior",
                                 text.less.than="menor que",
                                 fun=function(x) formatC(x, digits=0, format="d", big.mark = ",")),
            position=c("left", "top"), compass.type="8star")  

# tmap_mode("plot")
mapa.etaria.regsaude1

  # jpeg("mapa.etaria.regsaude1.jpeg", units="in", width=15, height=5, res=600)
  #  mapa.etaria.regsaude1
  #  dev.off()
...
Hospitais Regiões de Saúde {data-navmenu="Mapas Regiões de Saúde"}
=======================================================================
...
Column
-----------------------------------------------------------------------
### Leitos por 10.000 hab. Regiões de Saúde
...
Column
-----------------------------------------------------------------------
### Quociente Locacional Regiões de Saúde
...
# tab.leitos.regsaude.uti.ad$POP.SEM.UTI = tab.leitos.regsaude.uti.ad$POPULACAO.TOTAL.REG - tab.leitos.regsaude.uti.ad$POPULACAO
         ...
         # "P.Faixa.de00a14anos","P.adultos","P.Faixa.de60anosemais"

tab.leitos.regsaude.uti.ad = tab.etaria.regsaude %>%
  dplyr::left_join(tab.leitos.regsaude.uti.ad, by="REGSAUDE") %>%
    rename(POPULACAO = "POPULACAO.x",
         POPULACAO.AT = "POPULACAO.y")
...
tab.leitos.regsaude.uti.ad = mapas.leitos.regsaude %>%
  ...
  rename(POPULACAO = "POPULACAO.x") %>%
  rowwise() %>% 
  mutate(TOTAL10MIL.A = round(sum(QTEXIST, na.rm = TRUE)*10000/as.numeric(T.adultos),2),
            TOTAL10MIL.A.SUS = round(sum(QTSUS, na.rm = TRUE)*10000/as.numeric(T.adultos),2),
            TOTAL10MIL.A.NSUS = round(sum(QTNSUS, na.rm = TRUE)*10000/as.numeric(T.adultos),2)
            
  )

tab.leitos.regsaude.uti.ad = data.frame(tab.leitos.regsaude.uti.ad)
tab.leitos.regsaude.uti.ad$geometry = NULL
tab.leitos.regsaude.uti.ad$POPULACAO.y = NULL

tab.leitos.regsaude.uti.ad$QTSUS = ifelse(tab.leitos.regsaude.uti.ad$QTSUS == 0, NA,tab.leitos.regsaude.uti.ad$QTSUS )
tab.leitos.regsaude.uti.ad$QTNSUS = ifelse(tab.leitos.regsaude.uti.ad$QTNSUS == 0, NA,tab.leitos.regsaude.uti.ad$QTNSUS )

tab.leitos.regsaude.uti.ad$TOTAL10MIL.A = ifelse(tab.leitos.regsaude.uti.ad$TOTAL10MIL.A == 0, NA,tab.leitos.regsaude.uti.ad$TOTAL10MIL.A)
tab.leitos.regsaude.uti.ad$TOTAL10MIL.A.SUS = ifelse(tab.leitos.regsaude.uti.ad$TOTAL10MIL.A.SUS == 0, NA, tab.leitos.regsaude.uti.ad$TOTAL10MIL.A.SUS)
tab.leitos.regsaude.uti.ad$TOTAL10MIL.A.NSUS = ifelse(tab.leitos.regsaude.uti.ad$TOTAL10MIL.A.NSUS == 0, NA,tab.leitos.regsaude.uti.ad$TOTAL10MIL.A.NSUS)
# View(tab.leitos.regsaude.uti.ad)
...
mapa.leitos.regsaude.uti.ad1 = merge(mapas.leitos.regsaude, tab.leitos.regsaude.uti.ad, by="REGSAUDE", all.x=T)


mapa.leitos.regsaude.uti.ad11 =   tm_shape(mapars.leitos.regsaude, palette="Greys") +
   tm_borders("gray", lwd=0.1)+
tm_shape(mapa.leitos.regsaude.uti.ad1)+
tm_fill(c("QTEXIST","QTSUS","QTNSUS"), auto.palette.mapping=FALSE, id="REGSAUDE",
...
mapa.leitos.regsaude.uti.ad11
  
     # jpeg("mapa.leitos.regsaude.uti.ad11.jpeg", units="in", width=15, height=5, res=600)
     #  mapa.leitos.regsaude.uti.ad11
     #  dev.off()
```

Leitos UTI Adulto (I, II e III) para cada 10.000 adultos {data-navmenu="Mapas Regiões de Saúde"}
=======================================================================

Column
-----------------------------------------------------------------------
### Leitos UTI Adulto (I, II e III) para cada 10.000 adultos 

```{r, echo=FALSE}
mapa.leitos.regsaude.uti.ad2 =   tm_shape(mapars.leitos.regsaude, palette="Greys") +
   tm_borders("gray", lwd=0.1)+
tm_shape(mapa.leitos.regsaude.uti.ad1)+
tm_fill(c("TOTAL10MIL.A"
          ,"TOTAL10MIL.A.SUS"
          ,"TOTAL10MIL.A.NSUS"), auto.palette.mapping=FALSE, id="REGSAUDE",
          title=c("Leitos UTI Adulto \npor 10.000 hab. adultos",
                  "Leitos UTI Adulto SUS \npor 10.000 hab. adultos", 
                  "Leitos UTI Adulto não SUS \npor 10.000 hab. adultos"),
            breaks=c(1,2,3),
        textNA = "Zero", colorNA = "red",
...
tm_compass(position=c("right", "bottom"))+
tm_scale_bar(position=c("left", "bottom"))+
tm_text(c("TOTAL10MIL.A"
          ,"TOTAL10MIL.A.SUS"
          ,"TOTAL10MIL.A.NSUS"), scale=.7, )+
tm_borders(alpha=.5)+
tm_layout(
  ...
  tm_legend(legend.format = list(text.separator= "a",
                                 text.or.more="ou maior",
                                 text.less.than="menor que", 
                                 fun=function(x) formatC(x, digits=0.0, format="f")),
          position=c("left", "top"), compass.type="8star")


# View(tab.leitos.regsaude.uti.ad %>% dplyr::select("REGSAUDE","TOTAL10MIL.A","TOTAL10MIL.A.SUS","TOTAL10MIL.A.NSUS"))

# tab.leitos.regsaude.uti.ad

mapa.leitos.regsaude.uti.ad2

     # jpeg("mapa.leitos.regsaude.uti.ad2.jpeg", units="in", width=15, height=5, res=600)
     #  mapa.leitos.regsaude.uti.ad2
     #  dev.off()
```


```{r tab.equip.resp.regsaude, include=FALSE}
tab.equip.resp.regsaude = equip.cnes %>% 
    dplyr::select("CODUFMUN","MUNICIPIO","QTEXIST","QTUSO","INDSUS","INDNSUS","POPULACAO","REGSAUDE","EQUIPAMENTO") %>%
    filter(str_starts(EQUIPAMENTO, "RESPIRADOR/VENTILADOR")) %>%
    group_by(REGSAUDE) %>%
    summarise("QTEXIST" = sum(QTEXIST),
...
library(spdplyr)
mapa.equip.resp.regsaude = merge(mapas.leitos.regsaude, tab.equip.resp.regsaude, by="REGSAUDE", all.x=T)

mapa.equip.resp.regsaude = mapa.equip.resp.regsaude %>%  
  rowwise() %>% 
      mutate(TOTAL10MIL = round(sum(QTEXIST)*10000/as.numeric(POPULACAO),2),
              TOTAL10MIL.INDSUS = round(sum(INDSUS)*10000/as.numeric(POPULACAO),2),
              TOTAL10MIL.INDNSUS= round(sum(INDNSUS)*10000/as.numeric(POPULACAO),2))

mapa.equip.resp.regsaude = st_as_sf(mapa.equip.resp.regsaude)
# mapa.equip.resp.regsaude$geometry = NULL
# mapa.equip.resp.regsaude$POPULACAO.y = NULL
```


Respiradores por Região de Saúde {data-navmenu="Mapas Regiões de Saúde"}
=======================================================================

Column
-----------------------------------------------------------------------
### Respiradores por Região de Saúde

```{r, echo=FALSE}
mapa.equip.resp.regsaude1  =   tm_shape(mapars.leitos.regsaude, palette="Greys") +
   tm_borders("gray", lwd=0.1)+
tm_shape(mapa.equip.resp.regsaude)+
tm_fill(c("QTEXIST","TOTAL10MIL"),  id="REGSAUDE",
          title=c("Número de Respiradores/Ventiladores","Número de Respiradores/Ventiladores\n por 10.000 hab."),
            # breaks=c(1,2,3),
        textNA = "Zero", colorNA = "red",
          palette="Blues", alpha = .8,
          style="cont")+
...
tm_compass(position=c("right", "bottom"))+
tm_scale_bar(position=c("left", "bottom"))+
tm_text(c("QTEXIST","TOTAL10MIL"), scale=.7, )+
tm_borders(alpha=.5)+
tm_layout(
...
mapa.equip.resp.regsaude1

     # jpeg("mmapa.equip.resp.regsaude1.jpeg", units="in", width=15, height=5, res=600)
     #  mapa.equip.resp.regsaude1
     #  dev.off()
```
...
**Autores**
 ...
 - Iara Denise Endruweit Battisti ([*Lattes*](http://buscatextual.cnpq.br/buscatextual/visualizacv.do?id=K4798297H5))

 - Edemar Rotta ([*Lattes*](http://lattes.cnpq.br/9661112584933921))

 - Rafael Marcelo Soder ([*Lattes*](http://lattes.cnpq.br/3256384155681177))

 - Karina Kucharski ([*Lattes*](http://lattes.cnpq.br/8368653290392866))
 
 [**Nota técnica**](https://www.researchgate.net/publication/341651488_Disponibilidade_de_leitos_hospitalares_e_ventilacao_mecanica_no_Rio_Grande_do_Sul_desafios_no_enfrentamento_da_COVID-19?channel=doi&linkId=5ed45138299bf1c67d320aeb&showFulltext=true)
